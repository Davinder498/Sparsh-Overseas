...
export default function SparshSubmissionModal({ isOpen, onClose, title, description, onAuthorize, onSubmit, submissionStatus, errorMessage }: Props) {
    ...
    useEffect(() => {
        if (isOpen) setHasToken(!!sessionStorage.getItem('google_access_token'));
    }, [isOpen]);
    ...
    const handleAuthorize = async () => {
        ...
        try {
            await onAuthorize();
            setHasToken(true);
        } catch (error) {} finally { setIsAuthorizing(false); }
    };
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 dark:bg-black/70 flex items-center justify-center z-50 p-4 transition-opacity">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full p-6 text-center relative border dark:border-gray-700 animate-fade-in-up">
                ...
                {submissionStatus === 'SUCCESS' ? (
                    ...
                    <>
                        <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-primary-soft dark:bg-primary/20 mb-4">
                            <ShieldCheck className="h-6 w-6 text-primary" />
                        </div>
                        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">{title}</h3>
                        <div className="mt-4 text-sm text-gray-600 dark:text-gray-300">{description}</div>

                        {!hasToken && submissionStatus === 'IDLE' && (
                             <div className="mt-4 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-600/50 text-amber-800 dark:text-amber-300 px-4 py-3 rounded text-sm text-left flex flex-col items-start gap-3">
                                <div className="flex items-start">
                                    ...
                                    <div>
                                        <p className="font-bold">Google Authorization Required</p>
                                        <p className="mt-1">Grant permission for this app to send emails on your behalf.</p>
                                    </div>
                                </div>
                                <button onClick={handleAuthorize} disabled={isAuthorizing} className="w-full inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary hover:bg-primary-dark transition-colors disabled:opacity-50">
                                    ...
                                    {isAuthorizing ? 'Authorizing...' : 'Authorize Google'}
                                </button>
                            </div>
                        )}
                        ...
                        <div className="mt-6">
                            {(submissionStatus === 'IDLE' || submissionStatus === 'ERROR') ? (
                                <div className="flex flex-col-reverse sm:flex-row sm:justify-center sm:space-x-4 space-y-2 space-y-reverse sm:space-y-0">
                                    <button onClick={onClose} className="w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-base font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors sm:w-auto sm:text-sm">Cancel</button>
                                    <button onClick={onSubmit} disabled={!hasToken} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary-dark transition-colors sm:w-auto sm:text-sm disabled:opacity-50">
                                        <Send className="h-5 w-5 mr-2" /> Submit Now
                                    </button>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center justify-center py-4">
                                    <Loader2 className="animate-spin h-8 w-8 text-primary mb-2" />
                                    <span className="text-sm font-medium text-gray-600 dark:text-gray-300">{getLoadingMessage()}</span>
                                </div>
                            )}
                        </div>
                    </>
